...
export default function BarList({ data, labelKey = "_id", valueKey = "count" }) {
  if (!data?.length) return <p className="text-sm text-ink-soft">No data yet.</p>;
  const max = Math.max(...data.map((d) => d[valueKey]));
  return (
    <div className="space-y-2.5">
      {data.map((d) => (
        <div key={d[labelKey]} className="flex items-center gap-3">
          <span className="w-28 shrink-0 text-xs text-ink-soft">{d[labelKey]}</span>
          <div className="h-2.5 flex-1 rounded-sm bg-paper">
            <div
              className="h-2.5 rounded-sm bg-steel"
              style={{ width: max ? `${(d[valueKey] / max) * 100}%` : "0%" }}
            />
          </div>
          <span className="font-ref w-8 text-right text-xs text-ink">{d[valueKey]}</span>
        </div>
      ))}
    </div>
  );
